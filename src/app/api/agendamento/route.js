...
export async function GET() {
  try {
    ...
    const [linhas] = await pool.query(
      `SELECT id_agendamento, id_cliente, id_servico, data_agendamento, data_cadastro, observacao 
       FROM agendamento ORDER BY data_agendamento ASC, data_cadastro DESC`
    );
...
export async function POST(request) {
  ...
  try {
    ...
    const [clienteResult] = await pool.query(
      ...
      [dados.nome.trim(), dados.telefone.trim()]
    );
    const id_cliente = clienteResult.insertId;

    let id_servico = 1;
    if (dados.servico === 'Barba') {
      id_servico = 2;
    } else if (dados.servico === 'Combo completo') {
      id_servico = 3;
    }

    const [resultado] = await pool.query(
      `INSERT INTO agendamento (id_cliente, id_servico, data_agendamento, observacao)
      VALUES (?, ?, ?, ?)`,
      [id_cliente, id_servico, dados.data, dados.observacoes?.trim() || null]
    );
